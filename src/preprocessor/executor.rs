...
use super::{
    ...
    parser::Parser,
    past::{
        IfClause, IfCondition, IfStatement, Include, MLMacroDef, MLMacroUndef, MacroInvocation,
...
impl<'a> Executor<'a> {
    ...
    fn execute_nodes(&mut self, nodes: Vec<PASTNode>) -> EResult<Vec<Token>> {
        ...
        //         PASTNode::InertTokens(tokens) => Some(tokens.tokens),
        ...
        //         }
        //         PASTNode::MacroInvocation(macro_invocation) => self.execute_macro_invocation(macro_invocation)?,
        //     } {
    ...
    //                 if let PASTNode::InertTokens(inert_tokens) = node {
    //                     let mut new_inert_tokens = Vec::new();

    //                     for token in &inert_tokens.tokens {
    ...
    //                                     new_inert_tokens.push(*replacement_token);
    //                                 }
    //                             } else {
    //                                 new_inert_tokens.push(*token);
    //                             }
    //                         } else {
    //                             new_inert_tokens.push(*token);
    //                         }
    ...
    //                     cleaner_contents.push(PASTNode::InertTokens(InertTokens::from_vec(
    //                         new_inert_tokens,
    //                     )));
    //                 } else {
    ...
    //                             Ok(num) => num,
    //                             Err(_) => {
    //                                 self.session.struct_bug("Allowed macro argument index out of bounds for i32".to_string()).emit();
    //                                 return Err(());
    //                             }
    //                         };

    //                         if arg_ref == 0 {
    //                             self.session.struct_bug("Allowed invalid macro argument index of 0".to_string()).emit();
    //                             return Err(());
    //                         }
    ...
    //                             }
    //                         } else {
    //                             self.session.struct_bug("Allowed invalid macro argument index".to_string()).emit();
    //                             return Err(());
    //                         }
    //                     } else {
    //                         new_inert_tokens.push(*token);
    ...
    // fn execute_macro_invocation(&mut self, macro_invocation: MacroInvocation) -> EMaybe {
    //     let invocation_args = if let Some(args) = &macro_invocation.args {
    //         args.args.clone()
    //     } else {
    //         Vec::new()
    //     };

    //     let num_args_provided = invocation_args.len();
    ...
    //     for node in invocation_args {
    //         let tokens = self.execute_nodes(node.contents)?;

    //         arg_replacements.push(tokens);
    //     }

    //     if let Some(sl_macro) = self.sl_macros.get(&macro_invocation) {
    ...
    //             Ok(None)
    //         }
    //     } else if let Some(ml_macro) = self.ml_macros.get(&macro_invocation) {
    //         let new_contents =
    ...
    //         }
    //     } else {
    //         let macro_name_snippet = self.session.span_to_snippet(&macro_invocation.identifier.span);
    ...
    //                 macro_invocation.identifier.span,
    //                 format!(
    ...
    //                 .sl_macros
    //                 .get_accepted_num_args(macro_invocation.identifier.hash)
    //             {
    //                 db.note(format!(
    ...
    //                 Err(())
    //             } else {
    //                 let file_id = macro_invocation.identifier.span.file as u8;
    //                 let source_index = macro_invocation.identifier.span.start as u32;
    //                 let len = (macro_invocation.identifier.span.end - source_index as usize) as u16;
